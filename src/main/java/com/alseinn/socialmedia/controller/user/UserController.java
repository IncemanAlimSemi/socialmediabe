...
public class UserController {
    ...
    public ResponseEntity<UserFollowersResponse> getFollowers(@PathVariable String username) {
        return ResponseEntity.ok(userService.getFollowers(username));
    }

    @GetMapping("/followings/{username}")
    public ResponseEntity<UserFollowingsResponse> getFollowings(@PathVariable String username) {
